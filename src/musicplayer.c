...
// App states
typedef enum {
    STATE_MENU = 0,         // Main menu (Files / Radio / YouTube)
...
// Menu items
static const char* menu_items[] = {"Local Files", "Internet Radio", "MP3 Downloader"};
...
static void render_menu(void) {
    ...
    for (int i = 0; i < MENU_ITEM_COUNT; i++) {
        ...
        SDL_Color text_color = selected ? COLOR_BLACK : COLOR_WHITE;
        SDL_Surface* text = TTF_RenderUTF8_Blended(font.large, menu_items[i], text_color);
        if (text) {
